...
##### 1.1.2.1.1 skill-creator の内部プロセス
...
### 3.1.3 Superpowers — エージェントに「段取り」を教えるプラグイン
...
#### 3.1.5.2 VS Code + GitHub Copilot での使い方
...
### 3.1.11 GitHub Copilot Skills との関係
...
### 3.2.2 gstack — AIエージェントを「仮想チーム」に変える
...
### 3.3.2 課題1: 理解のずれ（Context Misalignment）
...
### 3.3.3 課題2: 実行失敗（Execution Failure）
...
### 3.3.4 課題3: 構造の問題（Structural Degradation）
...
## 3.4 3-3: frontend-design — フロントエンド設計支援スキル（Anthropic 公式）

> **学習時間**: 15分 | **難易度**: ⭐⭐

### 3.4.1 Copilot だけで頼むと
...
→ [3-6: ui-ux-pro-max — UI/UX最適化スキル](06-ui-ux-pro-max.md) で、設計したUIの品質を監査する方法を学ぶ
→ [4-9: 問題 × スキル解決マッピング](../04-skills-in-practice/09-problem-skill-mapping.md) で、全スキルの関係性を整理する

## 3.5 3-4: ui-ux-pro-max — UI/UX最適化スキル（コミュニティ）

> **学習時間**: 15分 | **難易度**: ⭐⭐

### 3.5.1 AIが生成したUIに、見えない問題が潜んでいる
...
## 4.1 4-1: grill-me — コードレビュースキル（Matt Pocock）
...
## 4.2 4-2: triage — Issue 分析スキル（Matt Pocock）
...
## 4.3 4-3: improve — コード改善スキル（Matt Pocock）
...
## 4.4 4-6: baoyu-diagram — SVG図形生成スキル
...
## 4.5 4-7: baoyu-infographic — インフォグラフィック生成スキル
...
### 4.5.10 この SKILL.md から学べる設計パターン
...
## 4.6 4-8: baoyu-comic — 知識マンガ生成スキル
...
### 4.6.12 この SKILL.md から学べる設計パターン
...
#### 4.7.5.1 問題1: 理解のずれ → grill-me + triage
...
#### 4.7.5.2 問題2: 実行失敗 → grill-me + improve
...
#### 4.7.5.3 問題3: 構造の問題 → frontend-design + improve
...
#### 4.7.5.4 問題4: UI/UX品質 → ui-ux-pro-max
...
#### 4.7.5.5 問題5: ビジュアルコンテンツ生成 → baoyu スキル群
...
Part 4 の学習が完了しました。Part 5（コンテンツ生成）に進みましょう：

→ [Part 5: baoyu-skills エコシステム入門](../05-content-creation/01-baoyu-ecosystem.md)
...
### 5.1.7 baoyu-skills のアーキテクチャ

#### 5.1.7.1 全体構成（3層アーキテクチャ）
...
#### 5.1.7.2 スキルの自己完結性（Self-Containment）
...
#### 5.1.7.3 インライン化ルール（必須セクションの自己完結）
...
#### 5.1.7.4 Progressive Disclosure（段階的開示）
...
### 5.1.11 baoyu-skills から学ぶ設計の教訓

#### 5.1.11.1 1. ドメイン特化の重要性
...
# 7.1 7-1: サンプル文書 — 品質改善 施策検討

> **種別**: 参考資料 | **役割**: Part 7 のユースケース背景 / 変換サンプル入力

## 7.1.1 このファイルの位置づけ

このドキュメントは、Part 7「ドキュメントワークフロー自動化」の**ユースケース背景**として用意された実務文書のサンプルです。

以下の2つの役割を持ちます。

| 役割 | 使われ方 |
|------|---------|
| **ユースケース背景** | なぜ Office ↔ Markdown 変換が必要かを示す実例として、[7-2](02-office-to-markdown.md) / [7-3](03-markdown-to-office.md) で引用 |
| **変換サンプル** | Pandoc による Markdown → Word 変換の入力ファイルとして [7-3](03-markdown-to-office.md) の実習で使用 |
...
### 7.1.2.4 3.1 仕様書運用方針（Copilot 活用前提）
...
→ [7-2: Office → Markdown 変換スキル（MarkItDown）](02-office-to-markdown.md)

# 7.2 7-2: Office → Markdown 変換スキル（MarkItDown）
...
## 7.2.4 実習: サンプルファイルを変換してみる

### 7.2.4.1 Word 文書の変換（sample2020.docx）
...
### 7.2.4.2 Excel ファイルの変換（Financial Sample.xlsx）
...
### 7.2.5.1 LLM ビジョンで alt テキストを生成する（方針 B）
...
## 7.2.6 発展: グラフを PNG として保持するパイプライン
...
flowchart TD
    ...
    Output["最終 Markdown\n![fig1](images/07-document-workflow/images/fig1\.png) として実ファイル参照"]
...
2. **重要なグラフだけ手動でスクリーンショット**して `assets/` に保存し `![説明](images/07-document-workflow/assets/chart\.png)` として追記
...
# 7.3 7-3: Markdown → Office 変換スキル（Pandoc）
...
```
@markdown-to-office
ファイル: docs/07-document-workflow/01-quality-improvement-plan.md
出力形式: docx
...
## 7.3.4 実習: 01-quality-improvement-plan.md を Word に変換する

### 7.3.4.1 ステップ 1: Pandoc のインストール確認

```bash
pandoc --version
```

### 7.3.4.2 ステップ 2: サンプルファイルを変換

```bash
cd docs/07-document-workflow
pandoc 01-quality-improvement-plan.md -o quality-improvement-plan.docx
```

### 7.3.4.3 ステップ 3: テンプレートを使って体裁を統一する
...
- [Part 6-1: パイプライン連携](../06-advanced/01-pipeline-integration.md) でこの2つのスキルをパイプラインに組み込む方法を学ぶ
...
## 8.1.3 スキル名 — バンドルスキル（Claude Code 標準搭載）
