# KDP Registration Package: Agent Skills in Practice

- Generated: 2026-06-27 09:58:23
- Project: agent-skills-in-practice
- Metadata: C:\Dev\tutorials\agent-skills-in-practice\.\.github\skills-config\ebook-build\agent-skills-in-practice.metadata.yaml
- KDP Metadata: C:\Dev\tutorials\agent-skills-in-practice\.\.github\skills-config\ebook-build\agent-skills-in-practice.kdp.yaml

## 1. Bibliographic Information

| Field | Value |
|---|---|
| Title | Agent Skills in Practice |
| Subtitle | スキルを作り、見つけ、活用する — Claude Code と GitHub Copilot で始める実践ガイド |
| Author | 杜 文吉 |
| Language | ja-JP |
| Publisher | Duwenji Lab |
| Rights | Copyright (c) 杜 文吉 |
| Identifier | agent-skills-in-practice |
| Publication Date | 2026-06-15 |
| ISBN | Typically not required for Kindle only |

## 2. Product Description

> Agent Skills オープンスタンダードに基づき、Claude Code と GitHub Copilot の両方でスキルを開発・活用する実践ガイド。 スキル作成から発見・共有、実践実装、コンテンツ生成まで体系的に学べます。

## 3. Keywords (up to 7)

- Agent Skills
- Claude Code
- GitHub Copilot
- AIコーディング
- スキル開発
- 開発効率化

## 4. Categories (up to 3)

- Computers / Software Development
- Computers / Programming

## 5. Pricing and Distribution

| Field | Value |
|---|---|
| List Price | 1490 JPY |
| Royalty | 70% |
| Territories | Worldwide |
| KDP Select | No |

## 6. Upload Files

- EPUB: C:\Dev\tutorials\agent-skills-in-practice\.\ebook-output\agent-skills-in-practice.epub
- PDF: C:\Dev\tutorials\agent-skills-in-practice\.\ebook-output\agent-skills-in-practice.pdf
