...
1. **Claude Code の `/run-skill-generator`**: 対話形式でスキルを生成。プロジェクトのビルド/起動方法を記録し、`.claude/skills/run-<name>/` にスキルとして保存します。
...
この教材のメインコンテンツです。フロントエンド開発に特化した5つの実践スキルを学びます：
...
| **対話生成** | `/run-skill-generator`（バンドルスキル） | なし（手書きが基本） |
...
| スキル生成 | SKILL.md/JSONを手書き | **Claude Code `/run-skill-generator`** + 手書き |
