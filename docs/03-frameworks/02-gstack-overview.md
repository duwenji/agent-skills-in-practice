...
### 計画・設計フェーズ（Think → Plan）

| スキル | 役割 | 説明 |
|--------|------|------|
| `/office-hours` | **YC オフィスアワー** | 6つの強制質問でプロダクトを再定義。コードを書く前に「何を作るべきか」を問い直す |
| `/plan-ceo-review` | **CEO / 創業者** | 問題を再考。4つのモード（拡張/選択的拡張/スコープ維持/削減）でリクエストの裏にある真の価値を探る |
| `/plan-eng-review` | **エンジニアリングマネージャー** | アーキテクチャ、データフロー、エッジケース、テストを確定。暗黙の前提を強制的に可視化する |
| `/plan-design-review` | **シニアデザイナー** | 各デザイン軸を0-10で評価。AIスロップ（品質の低いAI生成物）を検出する |
| `/plan-devex-review` | **DX リード** | 開発者体験の観点からレビュー。競合とのTTHW（Time To Hello World）比較、摩擦ポイントの特定 |
| `/design-consultation` | **デザインパートナー** | ゼロからデザインシステムを構築。ランドスケープ調査、クリエイティブリスクの提案、モックアップ生成 |
| `/autoplan` | **レビューパイプライン** | 1コマンドで完全レビュー済み計画を生成。CEO→デザイン→エンジニアレビューを自動実行 |

### 実装・レビューフェーズ（Build → Review）

| スキル | 役割 | 説明 |
|--------|------|------|
| `/review` | **スタッフエンジニア** | CIを通っても本番で壊れるバグを発見。明らかな問題は自動修正、ギャップはフラグ |
| `/investigate` | **デバッガー** | 体系的な根本原因デバッグ。鉄則：「調査なしに修正するな」。データフローを追跡し仮説を検証 |
| `/design-review` | **コードを書くデザイナー** | `/plan-design-review` と同じ監査を実行し、見つけた問題を実際に修正。アトミックコミット、before/afterスクリーンショット |
| `/devex-review` | **DX テスター** | 実際にオンボーディングをテスト。ドキュメントをナビゲートし、getting started フローを試し、TTHWを計測 |
| `/design-shotgun` | **デザイン探索者** | 「選択肢を見せて」。4-6種類のAIモックアップを生成し、ブラウザで比較ボードを開く。味覚記憶が学習 |
| `/design-html` | **デザインエンジニア** | モックアップを本番品質のHTMLに変換。Pretext レイアウトエンジン採用。30KB、ゼロ依存 |
| `/codex` | **セカンドオピニオン** | OpenAI Codex CLI による独立したコードレビュー。3モード：レビュー（pass/fail）、敵対的チャレンジ、オープン相談 |

### テスティング・QAフェーズ（Test）

| スキル | 役割 | 説明 |
|--------|------|------|
| `/qa` | **QA リード** | アプリをテストし、バグを見つけ、アトミックコミットで修正し、再検証。各修正に回帰テストを自動生成 |
| `/qa-only` | **QA レポーター** | `/qa` と同じ方法論だがレポートのみ。コード変更なしの純粋なバグレポート |
| `/benchmark` | **パフォーマンスエンジニア** | ページロード時間、Core Web Vitals、リソースサイズのベースライン計測。PRごとにbefore/afterを比較 |
| `/browse` | **QA エンジニア** | エージェントに「目」を与える。実際のChromiumブラウザを操作し、実際のクリック、実際のスクリーンショットを取得 |
| `/cso` | **チーフセキュリティオフィサー** | OWASP Top 10 + STRIDE 脅威モデル。ノイズゼロ：17のfalse positive除外、8/10以上の信頼度ゲート |

### デプロイ・リリースフェーズ（Ship）

| スキル | 役割 | 説明 |
|--------|------|------|
| `/ship` | **リリースエンジニア** | mainを同期、テスト実行、カバレッジ監査、push、PR作成。テストフレームワークがない場合は自動ブートストラップ |
| `/land-and-deploy` | **リリースエンジニア** | PRをマージ、CIを待ち、デプロイし、本番ヘルスを確認。「承認」から「本番確認済み」まで1コマンド |
| `/canary` | **SRE** | デプロイ後の監視ループ。コンソールエラー、パフォーマンス低下、ページ障害を監視 |
| `/document-release` | **テクニカルライター** | 出荷した変更に合わせて全プロジェクトドキュメントを更新。古くなったREADMEを自動検出 |
| `/document-generate` | **ドキュメンテーション著者** | Diataxis フレームワークを使用して不足ドキュメントをゼロから生成 |

### 振り返り・ユーティリティ（Reflect）

| スキル | 役割 | 説明 |
|--------|------|------|
| `/retro` | **エンジニアリングマネージャー** | チーム認識型の週次レトロスペクティブ。個人別 breakdown、出荷ストリーク、テスト健全性トレンド |
| `/learn` | **メモリー** | セッション間でgstackが学んだことを管理。プロジェクト固有のパターン、pitfall、設定を保存 |
| `/spec` | **スペック著者** | 曖昧な意図を正確で実行可能なスペックに変換。5フェーズ（why, scope, technical, draft, file） |
| `/pair-agent` | **マルチエージェント連携** | ブラウザを任意のAIエージェントと共有。OpenClaw, Hermes, Codex, Cursor などと連携可能 |

### パワーツール

| スキル | 説明 |
|--------|------|
| `/careful` | **セーフティガードレール** — 破壊的コマンド（rm -rf, DROP TABLE, force-push）の前に警告 |
| `/freeze` | **編集ロック** — ファイル編集を1つのディレクトリに制限。デバッグ中の意図しない変更を防止 |
| `/guard` | **フルセーフティ** — `/careful` + `/freeze` を1コマンドで有効化 |
| `/unfreeze` | **アンロック** — `/freeze` の境界を解除 |
| `/open-gstack-browser` | **GStack ブラウザ** — サイドバー、アンチボットステルス、自動モデルルーティングを備えた専用ブラウザを起動 |
| `/setup-deploy` | **デプロイ設定** — `/land-and-deploy` のための1回限りのセットアップ |
| `/setup-gbrain` | **GBrain オンボーディング** — 永続的知識ベースのセットアップ |
| `/gstack-upgrade` | **セルフアップデーター** — gstack を最新版にアップグレード |

### iOS スキル（v1.43.0.0+）

| スキル | 説明 |
|--------|------|
| `/ios-qa` | 実iPhoneをUSB CoreDevice経由で操作。iOSアプリの実機QAを実現 |
| `/ios-fix` | iOSバグ修正ループ |
| `/ios-design-review` | HIG準拠監査 |
| `/ios-clean` | デバッグブリッジのクリーンアップ |
