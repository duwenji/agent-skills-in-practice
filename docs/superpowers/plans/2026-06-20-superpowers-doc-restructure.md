# 01-superpowers.md リストラクチャリング 実装計画

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `docs/03-frameworks/01-superpowers.md` を物語構造に再構成し、初心者・中級者が冒頭から「これは自分の問題への答えだ」と感じながら読み進められるようにする。

**Architecture:** 既存コンテンツの削除はゼロ。「共感フック → 原因説明 → 解決策 → Before/After → ワークフロー → インストール手順 → スキル一覧 → 補足情報」の順に再配置する。①②のみ新規執筆。③は概要セクションを書き換え＋用語テーブルを統合。

**Tech Stack:** Markdown のみ（コードなし）

## Global Constraints

- 対象ファイルは `docs/03-frameworks/01-superpowers.md` 1ファイルのみ
- 既存の情報（インストール手順・スキル一覧・比較表・次のステップリンク）をすべて保持すること
...
**Files:**
- Modify: `docs/03-frameworks/01-superpowers.md`

**Interfaces:**
- Consumes: なし（単体ファイル）
- Produces: 新構成の `docs/03-frameworks/01-superpowers.md`

- [ ] **Step 1: 現在のファイル内容を確認し、保持すべき情報をメモする**

  読むべきセクション（全部保持する）：
  - インストールコマンド・手順（Claude Code / GitHub Copilot CLI）
  - 動作イメージのコード例（コマンドライン出力例）
  - 用語説明テーブル（6行）
  - 対応エージェント一覧テーブル
  - スキル一覧（各スキルの説明文）
  - 哲学テーブル・自動起動の説明
  - GitHub Copilot Skills との比較テーブル
  - このチュートリアルでの位置づけ（3項目）
  - 次のステップのリンク

- [ ] **Step 2: ファイルを新構成で上書き書き込む**

  以下の内容で `docs/03-frameworks/01-superpowers.md` を上書きする：
...
- [ ] **Step 3: 保持すべき情報がすべて含まれているか確認する**
  ...
  以下をファイル内で確認する（各項目を目視チェック）：
  - [ ] `superpowers@claude-plugins-official` のインストールコマンドが残っている
  - [ ] `~/.claude/settings.json` の JSON サンプルが残っている
  - [ ] `npm install -g @githubnext/github-copilot-cli` のコマンドが残っている
  - [ ] 対応エージェント一覧テーブル（8行）が残っている
  - [ ] 用語説明テーブル（Plugin, Marketplace, Skill, Subagent, HARD-GATE, Worktree）が残っている
  - [ ] スキル一覧（brainstorming, writing-plans, executing-plans, subagent-driven-development, dispatching-parallel-agents, test-driven-development, systematic-debugging, verification-before-completion, requesting-code-review, receiving-code-review, using-git-worktrees, finishing-a-development-branch, writing-skills, using-superpowers）が残っている
  - [ ] 哲学テーブル（4原則）が残っている
  - [ ] GitHub Copilot Skills との比較テーブルが残っている
  - [ ] 次のステップのリンク `02-gstack-overview.md` が残っている

- [ ] **Step 4: 新規追加されたセクションが存在するか確認する**

  - [ ] `## エージェントが「暴走」したことはないですか？` が冒頭近くに存在する
  - [ ] シーン1・シーン2・シーン3の3つのシナリオが存在する
  - [ ] `## エージェントの標準動作は「即コード」` が存在する
  - [ ] Before/After 比較表（標準エージェント vs Superpowers 導入後）が存在する
  - [ ] `## 同じ依頼で何が変わるのか` セクションが存在する
  - [ ] Before（Superpowers なし）のターミナル出力例が存在する
  - [ ] スキル一覧の見出しが `## スキル一覧（リファレンス）` になっている
...
- [ ] **Step 5: コミットする**
  ...
  git commit -m "docs: 01-superpowers.md を物語構造に再構成

  初心者・中級者が冒頭から価値訴求を感じながら読み進められるよう再設計。
  共感フック→原因説明→解決策→Before/Afterの順に変更。情報の削除はゼロ。"
  ```
...
**Files:**
- Modify: `docs/superpowers/plans/2026-06-20-superpowers-doc-restructure.md`（このファイル）
...
- [ ] **Step 1: 計画ファイルをコミットする**
  ...
  ```bash
  git add docs/superpowers/plans/2026-06-20-superpowers-doc-restructure.md
  git commit -m "docs: 01-superpowers.md リストラクチャリングの実装計画を追加"
  ```
