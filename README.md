...
| # | セクション | 内容 |
|---|-----------|------|
| 0-1 | Agent Skills エコシステムの全体像 | 本教材の目的、4層構造の解説 |
| 0-2 | 環境セットアップ | Claude Code / GitHub Copilot 両方の準備 |

### Part 1: スキル作成入門（60分）
| # | セクション | 内容 |
|---|-----------|------|
| 1-1 | Agent Skills とは | オープンスタンダード、Claude Code と GitHub Copilot の比較 |
| 1-2 | skill-creator で最初のスキルを作る | ハンズオン：対話形式でスキル生成、テスト・評価・改善サイクル |
| 1-3 | SKILL.md の手書きとカスタマイズ | 生成後編集・最適化、両プラットフォーム対応 |
| 1-4 | スキル作成のベストプラクティス | 効果的なプロンプト設計、反復改善サイクル |

### Part 2: スキル発見と共有（60分）
| # | セクション | 内容 |
|---|-----------|------|
| 2-1 | Find Skills / gh skill でスキルを探す | GitHub.com 上のスキル発見機能、CLI検索 |
| 2-2 | skill.sh によるクロスプラットフォーム検索 | コマンドラインからスキルを検索・実行 |
| 2-3 | スキルの共有とチーム展開 | Personal Skills / Project Skills / submodule 戦略 |
| 2-4 | baoyu-skills で学ぶ実践的スキル発見 | 21,000+ Stars の人気リポジトリを題材にスキル発見を実践 |
...
| 3-5 | baoyu-skills のアーキテクチャ分析 | 3大スキルセット比較、自己完結型設計、5Dスタイル体系 |

### Part 4: 実践スキル実装編（180分）⭐ メイン
| # | スキル | 種類 | 学習ポイント |
|---|-------|------|------------|
| 4-1 | **grill-me**（コードレビュー） | 品質検証 | コード分析、レビュー観点の自動化、SKILL.md実装 |
| 4-2 | **triage**（Issue分析） | 優先順位付け | Issue解析、ラベル分類、優先度判定ロジック |
| 4-3 | **improve**（コード改善） | リファクタリング | 改善提案、パフォーマンス最適化、リファクタリング支援 |
| 4-4 | **frontend-design**（設計支援） | アーキテクチャ | コンポーネント設計、状態管理設計、レンダリング最適化 |
| 4-5 | **ui-ux-pro-max**（UI/UX最適化） | デザイン改善 | アクセシビリティ、レスポンシブデザイン、ユーザビリティ監査 |
| 4-6 | **baoyu-diagram**（SVG図形生成） | コンテンツ生成 | SVG直書き、5図タイプ、ダークモード対応 |
...
### 3. 実践スキルを試す
`docs/04-skills-in-practice/` から使いたいスキルを選び、SKILL.md をコピーして配置します。
...
✓ Superpowers（obra/superpowers）の考え方を理解し、スキル設計に応用できる
...
👉 [Part 0: エコシステム概要を読む →](docs/00-fundamentals/01-ecosystem-overview.md)
