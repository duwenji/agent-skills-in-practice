# MASTER INDEX

## Cover
- `docs/00-COVER.md`

## Part 0: 概要と環境準備（20分）
- `docs/00-fundamentals/01-ecosystem-overview.md` — Agent Skills エコシステムの全体像
- `docs/00-fundamentals/02-environment-setup.md` — 環境セットアップ

## Part 1: スキル作成入門（60分）
- `docs/01-skill-creation/01-what-are-agent-skills.md` — Agent Skills とは
- `docs/01-skill-creation/02-skill-creator-hands-on.md` — Claude Code `/skill-creator` で最初のスキルを作る
- `docs/01-skill-creation/03-skillmd-customization.md` — SKILL.md の手書きとカスタマイズ
- `docs/01-skill-creation/04-best-practices.md` — スキル作成のベストプラクティス

## Part 2: スキル発見と共有（60分）
- `docs/02-discovery/01-find-skills.md` — Find Skills / gh skill でスキルを探す
- `docs/02-discovery/02-skill-sh-cli.md` — skill.sh によるクロスプラットフォーム検索
- `docs/02-discovery/03-sharing-team-deployment.md` — スキルの共有とチーム展開
- `docs/02-discovery/04-baoyu-skills-discovery.md` — baoyu-skills で学ぶ実践的スキル発見

## Part 3: 概念フレームワークと課題認識（80分）
- `docs/03-frameworks/01-superpowers.md` — Superpowers: コーディングエージェントの開発方法論
- `docs/03-frameworks/02-gstack-overview.md` — gstack: Garry Tan の Claude Code スキルセット
- `docs/03-frameworks/03-3a-frontend-design.md` — frontend-design（Anthropic公式）
- `docs/03-frameworks/03-3b-ui-ux-pro-max.md` — ui-ux-pro-max（コミュニティ）
- `docs/03-frameworks/04-problem-skill-mapping.md` — 問題 × スキル解決マッピング
- `docs/03-frameworks/05-baoyu-skills-architecture.md` — baoyu-skills のアーキテクチャ分析

## Part 4: 実践スキル実装編（180分）⭐
- `docs/04-skills-in-practice/01-grill-me.md` — grill-me（コードレビュー）
- `docs/04-skills-in-practice/02-triage-issue-analysis.md` — triage（Issue分析）
- `docs/04-skills-in-practice/03-improve.md` — improve（コード改善）
- `docs/04-skills-in-practice/04-frontend-design.md` — frontend-design（設計支援）
- `docs/04-skills-in-practice/05-ui-ux-pro-max.md` — ui-ux-pro-max（UI/UX最適化）
- `docs/04-skills-in-practice/06-baoyu-diagram.md` — baoyu-diagram（SVG図形生成）
- `docs/04-skills-in-practice/07-baoyu-infographic.md` — baoyu-infographic（インフォグラフィック）
- `docs/04-skills-in-practice/08-baoyu-comic.md` — baoyu-comic（知識マンガ生成）

## Part 5: コンテンツ生成スキル実践（90分）🆕
- `docs/05-content-creation/01-baoyu-ecosystem.md` — baoyu-skills エコシステム入門
- `docs/05-content-creation/02-content-skills-in-action.md` — コンテンツ生成スキルを使いこなす
- `docs/05-content-creation/03-image-gen-backends.md` — AI画像生成バックエンドの選択
- `docs/05-content-creation/04-skill-pipeline.md` — 実プロジェクトでのスキル連携
- `docs/05-content-creation/05-custom-skill-development.md` — カスタムスキル開発（baoyu流）

## Part 6: 発展・応用（65分）
- `docs/06-advanced/01-pipeline-integration.md` — 複数スキルの連携パイプライン
- `docs/06-advanced/02-ci-cd-integration.md` — 実プロジェクトへの組み込み
- `docs/06-advanced/03-evaluation-cycle.md` — スキル評価と改善サイクル
- `docs/06-advanced/04-skill-creator-deep-dive.md` — skill-creator 徹底解説（対話生成の仕組みと活用ノウハウ）

## Part 8: 用語集

- `docs/08-glossary/01-glossary.md` — 教材全体の用語集（用語種付き）

## Samples
- `samples/grill-me/SKILL.md`
- `samples/triage/SKILL.md`
- `samples/improve/SKILL.md`
- `samples/frontend-design/SKILL.md`
- `samples/ui-ux-pro-max/SKILL.md`
- `samples/baoyu-diagram/SKILL.md`（参考: [JimLiu/baoyu-skills](https://github.com/JimLiu/baoyu-skills)）
- `samples/baoyu-infographic/SKILL.md`（参考: [JimLiu/baoyu-skills](https://github.com/JimLiu/baoyu-skills)）
- `samples/baoyu-comic/SKILL.md`（参考: [JimLiu/baoyu-skills](https://github.com/JimLiu/baoyu-skills)）
